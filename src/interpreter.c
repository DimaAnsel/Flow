...
ErrCode cmd_cache(char expression[]) {
	...
	if (error == NO_ERROR) {
		if (var > LOADED_VAR) {
			(*var) = (unsigned char)(((long)((long)var - (long)LOADED_VAR))%0x100);
		} else {
			(*var) = (unsigned char)(((long)((long)LOADED_VAR - (long)var))%0x100);
		}
	}
	return error;
}
...
ErrCode translate_hex(char expression[], unsigned char* ret) {
	...
	if (expression[0] >= '0' && expression[0] <= '9') {
		a = (unsigned char)(expression[0] - '0');
	} else if (expression[0] >= 'A' && expression[0] <= 'F') {
		a = (unsigned char)(10 + (expression[0] - 'A'));
	} else {
...
ErrCode move_variable(char expression[], unsigned char** ret, char decrement) {
	...
	error = evaluate(expression, &temp);
	if (error == NO_ERROR) {
		unsigned char* dest = LOADED_VAR + (decrement ? -temp : temp);
